...
class ImageGenerator:
    ...
    def _query_hf_api(self, endpoint, payload):
        ...
        try:
            ...
            # A 503 response often means the model is loading, which is a common occurrence.
    ...
    def generate_image_from_hf(self, prompt: str):
        ...
        for endpoint in self.endpoints:
            ...
            if generated_image:
                return generated_image  # Return the first successfully generated image
...
def check_api_keys():
    """Checks if essential API keys are empty strings or placeholders. Provides warnings."""
    warnings = []
    if not OPENROUTER_API_KEY or OPENROUTER_API_KEY == "YOUR_OPENROUTER_DEEPSEEK_API_KEY":
        warnings.append("OPENROUTER_API_KEY (for Deepseek) is not set. AI text processing will fail.")
    if not OPENROUTER_MISTRAL_API_KEY or OPENROUTER_MISTRAL_API_KEY == "YOUR_OPENROUTER_MISTRAL_API_KEY":
        warnings.append("OPENROUTER_MISTRAL_API_KEY (for Mistral) is not set. Caption/hashtag generation and weekly analysis will fail.")
    if not HUGGING_FACE_TOKEN or HUGGING_FACE_TOKEN == "YOUR_HUGGING_FACE_TOKEN":
        warnings.append("HUGGING_FACE_TOKEN is not set. Image generation will be skipped.")
    if not PEXELS_API_KEY or PEXELS_API_KEY == "YOUR_PEXELS_API_KEY":
        warnings.append("PEXELS_API_KEY is not set. Pexels image fetches will fail.")
    if not UNSPLASH_ACCESS_KEY or UNSPLASH_ACCESS_KEY == "YOUR_UNSPLASH_ACCESS_KEY":
        warnings.append("UNSPLASH_ACCESS_KEY is not set. Unsplash image fetches will fail.")
    if not PIXABAY_API_KEY or PIXABAY_API_KEY == "YOUR_PIXABAY_API_KEY":
        warnings.append("PIXABAY_API_KEY is not set. Pixabay image fetches will fail.")
    if not CLOUDINARY_CLOUD_NAME or not CLOUDINARY_API_KEY or not CLOUDINARY_API_SECRET:
...
if __name__ == "__main__":
    ...
    image_generator = ImageGenerator(HUGGING_FACE_TOKEN, INFERENCE_API_ENDPOINTS) # NEW: For Hugging Face generation
    image_fetcher = ImageFetcher() # For stock photo APIs (fallback)
    ...
    try:
        ...
        # --- MODIFIED: Image Generation/Fetching Logic ---
        ...
        print("\nGenerating caption and hashtags with Mistral...")
        ...
        print("\nSaving post metadata and local image...")
        ...
        if post_to_process['final_image'] and os.path.exists(local_image_path_for_upload): # Check if image exists locally
            ...
            print("\nSkipping Cloudinary upload: No valid local image found at path or image not generated.")
        ...
        if cloudinary_image_url:
            ...
            print("\nSkipping Instagram post: No Cloudinary image URL available.")
        ...
        print(f"\nSuccessfully processed post {post_number_for_this_run}/{len(CONTENT_TYPE_CYCLE)}. State updated for next trigger.")
